...
public class Dealer extends JFrame {
	...
	private ArrayList<Playbj> players; //all the players that are connected 
	private Playbj dcards;
	private int playersleft; //When players leave
	...
	public void runDeal()
	{
		try 
		{
			server = new ServerSocket( 23555, 100 ); // created a ServerSocket
	...
	private void DealCards(){
         //This line of code is the dealer giving the cards to the players
		try{
			playersleft = counter-1;
			newdeck.Shuffle(); //It gives the players shuffled cards 
			dcard1 = newdeck.dealCards();
			dcard2 = newdeck.dealCards();
			displayMessage("\n\n" +dcard1 + " || " +dcard2); // Displays the cards u got given 

			for (int i=1;i<= counter;i++) {
				String c1,c2;
				c1 = newdeck.dealCards();
				c2 = newdeck.dealCards();
				Playbj p = new Playbj(c1,c2);
				players.add(p);
				sockServer[i].sendData("You were Dealt:\n" + c1 + " || " + c2); // the cards that given to the players
				sockServer[i].sendData("Your Total: " +  p.GetCardTotal()); //The total value of the cards the player received
	...
	private void Results() {
		...
		//This batch of code is to determine if the player has won or not 
		// if statements when dealer and player are under 21, it checks who has the higher value
	...
	private class SockServer implements Runnable
	{
		...
		{
			String message = "Player " + myConID + " successful";
		...
		private void DealerGo() {		
			...
			if (dcards.GetCardTotal() < 15){
				while(dcards.GetCardTotal() < 15){
					String card1 = newdeck.dealCards();
